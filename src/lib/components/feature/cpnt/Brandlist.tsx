...
const UseFlagSwitch = memo(({ isChecked, onChange }: { isChecked: boolean; onChange: () => void }) => (
...
export default function ListBrandlist({
  list,
  usePagination = false,
  pagination,
  setPagination,
  ...
  const columns: ColumnDef<ContentListResponse>[] = [
    {
      ...
      header: '컨텐츠 ID',
      cell: (row) => <CommonLinkId id={row.row.original.contentId.toString()} route="content" typeCode="BL" />,
    },
    {
      accessorKey: 'title',
      header: '제목',
    ...
    },
    {
      ...
      cell: (row) => (
        <UseFlagSwitch
          isChecked={row.row.original.useFlag}
          onChange={() => handleUseFlag(row.row.original.contentId, !row.row.original.useFlag)}
        />
      ),
    },
    {
