...
export default function ListQuickButton({
  list,
  usePagination = false,
  pagination,
  setPagination,
  ...
  const columns: ColumnDef<ContentListResponse>[] = [
    {
      ...
      header: '컨텐츠 ID',
      cell: (row) => <CommonLinkId id={row.row.original.contentId.toString()} route="content" typeCode="QB" />,
    },
    {
      accessorKey: 'imgLink',
      header: '이미지',
        ...
        <Popover>
          <PopoverTrigger>
            {row.row.original.imgLink ? (
              row.row.original.imgLink?.endsWith('.lottie') ? (
                <DotLottieReact
                  style={{ height: '64px', width: '64px' }}
                  autoplay
                  loop
                  src={row.row.original.imgLink}
                />
              ) : (
