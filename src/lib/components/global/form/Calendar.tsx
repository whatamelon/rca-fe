...
import { Button, buttonVariants } from '@/lib/components/element/button/Button'
...
function Calendar({
  ...
  const _dayButtonClassName = cn(
    buttonVariants({ fill: 'gray' }),
    'size-8 rounded-lg p-0 font-bold text-tiny transition-none aria-selected:opacity-100 ',
    props.dayButtonClassName,
  )
  const buttonRangeClassName =
    'bg-accent [&>button]:bg-blue-500 [&>button]:text-primary-foreground [&>button]:hover:bg-blue-500 [&>button]:hover:text-primary-foreground'
  const _rangeStartClassName = cn(
    buttonRangeClassName,
    'day-range-start rounded-s-lg !bg-blue-500',
    props.rangeStartClassName,
  )
  const _rangeEndClassName = cn(
    buttonRangeClassName,
    'day-range-end rounded-e-lg !bg-blue-500',
    props.rangeEndClassName,
  )
  const _rangeMiddleClassName = cn(
    'bg-blue-300 !text-blue-700 [&>button]:bg-transparent [&>button]:!text-blue-700',
    props.rangeMiddleClassName,
  )
  const _selectedClassName = cn(
    '[&>button]:bg-blue-500 [&>button]:text-white [&>button]:hover:bg-blue-500/30',
    props.selectedClassName,
  )
  const _todayClassName = cn('[&>button]:bg-gray-900 [&>button]:text-white', props.todayClassName)
